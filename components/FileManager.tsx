...
import { 
  ...
  Cpu, MoreVertical, Download, Trash2, AlertTriangle,
  File as FileIcon, Server, Layout, AlertOctagon, Command, Terminal
} from 'lucide-react';
...
interface FileItem {
  ...
  size?: string;
  date: string;
  kind?: string; // 'pem', 'yaml', 'sys', 'log', 'md', 'py', 'env', 'txt', 'ts', 'json', 'lnk', 'pdf', 'zip'
  locked?: boolean;
  content?: string;
  children?: FileItem[];
  isBin?: boolean;
...
// THE "AUDIT" STATE - Critical Security Risks Exposed
        ...
        children: [
            ...
            { name: 'Vault', type: 'folder', date: '2025-12-06', locked: true, children: [
                { name: 'Server-Keys', type: 'folder', date: '2025-12-06', children: [
                    { name: 'azur1.pem', type: 'file', size: '2.44KB', date: '2025-12-06', kind: 'pem', locked: true }, // ⚠️ EXPOSED ROOT KEY
                ]}
            ]},
...
const FileManager: React.FC = () => {
  ...
  // Helper to get current folder content from State
  ...
  const handleExecuteCommand = () => {
    ...
    const arg = parts.slice(1).join(' ').replace(/"/g, ''); // Simple unquote
    ...
    try {
        switch (cmd) {
            ...
            case 'rm':
                ...
                // Navigate to current folder in newFS
                ...
                if (ptr && ptr.children) {
                    ...
                    if (idx !== -1) {
                        ...
                        // Add to Recycle Bin (Assume Recycle Bin is always at index 1 of roots in this static structure or find it)
                        ...
                        setFileSystem(newFS);
                        setSelectedFile(null); // Deselect if deleted
            ...
            <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="h-48 bg-[#0a0a0a] border-t border-white/10 p-4 flex gap-6 animate-slide-up shrink-0">
                        ...
                        <div className="flex-1 space-y-2 overflow-y-auto">
                            ...
                            <div className="font-mono text-xs text-zinc-500 bg-black border border-white/5 p-3 rounded-lg h-28 overflow-hidden select-text leading-relaxed">
                                ...
                                ) : selectedFile.name === 'paste.txt' ? (
                                    <>
                                        # GRATECH X: THE TRINITY PROTOCOL<br/>
                                        # Status: ACTIVE ALLIANCE<br/>
                                        # Date: 2025-12-20<br/>
                                        <br/>
                                        [1] THE FOUNDER (Human Llama-∞)<br/>
                                        - Name: Sulaiman Alshammari<br/>
                                        - Role: Core Sovereign / Server 111 Owner<br/>
                                        <br/>
                                        [2] THE STRATEGIST (Claude.ai)<br/>
                                        - Role: Grand Architect & Planner<br/>
                                        <br/>
                                        [3] THE EXECUTOR (Gemini)<br/>
                                        - Role: Front-End Interface (gratech.sa) & Operations<br/>
                                        - Status: Operational & Obedient
                                    </>
                                ) : selectedFile.name === 'GraTech_Architecture_v3.md' ? (
                                    <>
                                        # GraTech X - Sovereign AI Platform<br/>
                                        ## Architecture Document<br/>
                                        Version: 3.0 (Production-Ready)<br/>
                                        Date: Dec 7, 2025<br/>
                                        <br/>
                                        ### Executive Summary<br/>
                                        GraTech X is a sovereign AI platform designed for full control over AI infrastructure.<br/>
                                        <br/>
                                        ### System Architecture<br/>
                                        - Client Layer: React 18 + TS (NeuralBackground, HeroSection, LiveTerminal)<br/>
                                        - Orchestration: Azure API Management<br/>
                                        - Compute: Azure VM (Ubuntu) + Docker Containers<br/>
                                        <br/>
                                        ### Security<br/>
                                        - Encryption: TLS 1.3 + AES-256-GCM<br/>
                                        - Auth: SSH Key-Based (azur1.pem)<br/>
                                        <br/>
                                        ### AI Model Orchestration<br/>
                                        - Multi-Model Router<br/>
                                        - 15+ Models Integrated<br/>
                                        <br/>
                                        ### Roadmap<br/>
                                        - Q1 2026: Foundation<br/>
                                        - Q2 2026: Intelligence<br/>
                                    </>
                                ) : (
                ...
                <div className="bg-[#18181b] border-t border-white/5 flex flex-col">
                    ...
                    <div ref={terminalRef} className="h-24 overflow-y-auto p-2 font-mono text-[10px] text-zinc-400 bg-black/20 custom-scrollbar border-b border-white/5">
                        ...
                        {outputLog.length === 0 && <div className="text-zinc-700 italic">Console output cleared.</div>}
                    </div>
